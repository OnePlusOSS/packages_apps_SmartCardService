...
public class AccessControlEnforcer {
    ...
    public synchronized void reset() {
        ...
        SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "Reset the ACE for terminal:" + mTerminal.getName());
        mAraController = null;
        mArfController = null;
    }
    	...
    	try {
	        ...
	        if( mUseAra && mAraController != null ){
	    		try {
	    			...
	    			SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, "ARA applet is used for:" + mTerminal.getName());
	    			mUseArf = false;
	    			mFullAccess = false;
	    			...
	    			denyMsg = e.getLocalizedMessage();
	                 ...
	                 if( e instanceof MissingResourceException ) {
	                     if(mTerminal.getName().startsWith(SmartcardService._UICC_TERMINAL)) {
	                         ...
	                         SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, "Got MissingResourceException: Does the UICC support logical channel?");
	                         SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, "Full message: " +  e.getMessage());
	                     } else {
	                	...
	                	SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, "No ARA applet found in: " + mTerminal.getName());
	    			} else {
	    				// ARA is available but doesn't work properly.
	    				// We are going to disable everything per security req.
	    				SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, "AccessControlEnforcer - Problem accessing ARA, Access DENIED. " + e.getLocalizedMessage());
	    	...
	    	if(mUseArf && !mTerminal.getName().startsWith(SmartcardService._UICC_TERMINAL)) {
	    		SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, "Disable ARF for terminal: " + mTerminal.getName() + " (ARF is only available for UICC)");
	        ...
	        if( mUseArf && mArfController != null) {
	    		try {
	    			...
	    			// disable other access methods
	    			SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, "ARF rules are used for:" + mTerminal.getName());
	    			mFullAccess = false;
	    		} catch( Exception e ) {
		    		...
		    		status = false;
	    			denyMsg = e.getLocalizedMessage();
		    		SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, e.getMessage() );
				}
	    	}
	        ...
	        if(mFullAccess) {
	            ...
	            SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, "Full access granted for:" + mTerminal.getName());
	        }
	        ...
	        if(!mUseArf && !mUseAra && !mFullAccess) {
	            ...
	            SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, "Deny any access to:" + mTerminal.getName());
	        }
	        mRulesRead = status;
	    	return status;
    	} finally {
	 	...
	 	for( String packageName : packageNames ) {
	 		// estimate SHA-1 hash value of the device application's certificate.
				...
				try {
					...
					SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, " Access Rules for NFC: " + e.getLocalizedMessage());
    ...
    private void readSecurityProfile() {
        ...
        SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "Allowed ACE mode: ara=" + mUseAra + " arf=" + mUseArf + " fullaccess=" + mFullAccess );
    }
