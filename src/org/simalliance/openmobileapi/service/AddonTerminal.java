...
public class AddonTerminal extends Terminal {
    ...
    /* + snb GetUid */
    private Method mInternalGetUid = null;
	/* - snb GetUid */
    ...
    public AddonTerminal(Context context, String packageName, String className) {
        ...
        try {
            ...
            if (mInstance != null) {
				...
				/* + snb GetUid */
                mInternalGetUid = mInstance.getClass().getDeclaredMethod("internalGetUid", (Class<?>[]) null);
				/* - snb GetUid */
    ...
    protected byte[] internalGetUid() throws CardException {
        if (mInternalGetUid == null) {
            throw new IllegalStateException(
                    "plugin error: Function String internalGetUid() not found");
        }
        try {
            byte[] resp = (byte[]) mInternalGetUid.invoke(mInstance, (Object[]) null);
            return resp;
        } catch (Exception e) {
            throw new CardException("plugin internal error: internalGetUid() execution: "
                    + e.getCause());
        }
    }
