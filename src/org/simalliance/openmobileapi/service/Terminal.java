...
public abstract class Terminal implements ITerminal {
   ...
   /**
    ...
    public void closeChannel(Channel channel) throws CardException {
        ...
        synchronized(mLock) {
		/* + snb log */
		SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "closeChannel start");
		/* - snb log */
	        try {
		...
		/* + snb log */
		SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "closeChannel end");
		/* - snb log */
        }
    }
	...
	/* + snb GetUid */
    /**
     * Implements the terminal specific getuid operation.
     * 
     * @return the response UID received.
     * @throws CardException if the transmit operation failed.
     */
    protected abstract byte[] internalGetUid() throws CardException;
	/* - snb GetUid */
    ...
    public Channel openBasicChannel(SmartcardServiceSession session, ISmartcardServiceCallback callback) throws CardException {
        ...
        synchronized (mLock) {
			/* + snb log */
        	SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "openBasicChannel start");
			/* - snb log */
			...
			/* + snb log */
            SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "openBasicChannel end");
			/* - snb log */
            return basicChannel;
        }
    }
    ...
    public Channel openBasicChannel(SmartcardServiceSession session, byte[] aid, ISmartcardServiceCallback callback) throws Exception {
        ...
        synchronized (mLock) {
			/* + snb log */
        	SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "openBasicChannel aid start");
			/* - snb log */
            if (getBasicChannel() != null) {
                throw new CardException("basic channel in use");
            }
            ...
            registerChannel(basicChannel);
			/* + snb log */
            SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "openBasicChannel aid end");
			/* - snb log */
            return basicChannel;
        }
    }
    ...
    public Channel openLogicalChannel(SmartcardServiceSession session, ISmartcardServiceCallback callback) throws Exception {
        ...
        synchronized (mLock) {
			/* + snb log */
        	SmartcardService.log(TERMINAL_TAG +SmartcardService._TAG +  SmartcardService.LOG_LEVEL_MIDDLE, "openLogicalChannel start");
			/* - snb log */
            if (mChannels.isEmpty()) {
                internalConnect();
            }
			...
			/* + snb log */
            SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "openLogicalChannel end");
			/* - snb log */
    ...
    public Channel openLogicalChannel(SmartcardServiceSession session, byte[] aid, ISmartcardServiceCallback callback) throws Exception {
        ...
        synchronized (mLock) {
			/* + snb log */
        	SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "openLogicalChannel aid start");
			/* - snb log */
            if (mChannels.isEmpty()) {
                internalConnect();
            }
            ...
            registerChannel(logicalChannel);
			/* + snb log */
            SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "openLogicalChannel aid end");
			/* - snb log */
            return logicalChannel;
        }
    }
	...
	/* + snb sync conflict */
	/* + snb sync conflict 2nd */
    protected synchronized byte[] TransmitEncapsulation(byte[] command) throws CardException {
	/* - snb sync conflict 2nd */
        byte[] l_command = command;
        byte[] rsp = null;
		/* + snb sync conflict 2nd */
        //synchronized (mLock) {	
		/* - snb sync conflict 2nd */
        	rsp = internalTransmit(l_command);
		/* + snb sync conflict 2nd */
        //}
		/* - snb sync conflict 2nd */
        return rsp;
	...
	/* + snb GetUid */
    public byte[] GetUidEncapsulation() throws CardException {
        byte[] rsp = null;
        synchronized (mLock) {	
        	rsp = internalGetUid();
        }
	...
	/* - snb sync conflict 2nd */
        ...
        */
        	rsp = TransmitEncapsulation(command);
		/* - snb sync conflict */
        ...
        if (rsp.length >= 2) {
            ...
            if (sw1 == 0x6C) {
                ...
                while (true) {
                    ...
                    rsp = TransmitEncapsulation(getResponseCmd);		
					/* - snb sync conflict */
    ...
    final class SmartcardServiceReader extends ISmartcardServiceReader.Stub {
		...
		public ISmartcardServiceSession openSession(SmartcardError error)
	        ...
	        synchronized (mLock) {
				/* + snb log */
	        	SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "openSession start");
				/* - snb log */
	        	try {
				...
				/* + snb log */
	            SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "openSession end");
				/* - snb log */
		...
		public void closeSessions(SmartcardError error) throws RemoteException {
	        ...
	        synchronized (mLock) {
				/* + snb log */
	        	SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "closeSessions start");
				/* - snb log */
	            ...
	            mSessions.clear();
				/* + snb log */
	            SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "closeSessions end");
				/* - snb log */
	        }
		}
	    ...
	    void closeSession(SmartcardServiceSession session) throws RemoteException, CardException {
	        ...
	        synchronized (mLock) {
				/* + snb log */
	        	SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "closeSession start");
				/* - snb log */
				if (!session.isClosed()) {
				...
				/* + snb log */
	            SmartcardService.log(TERMINAL_TAG + SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "closeSession end");
				/* - snb log */
	        }
	    }
