...
class Channel implements IChannel, IBinder.DeathRecipient {
	...
	/* - snb log */
	/* + snb for lingnan */
    private static final int mMaxChannelType =  4;
	/* - snb for lingnan */
    ...
    Channel(SmartcardServiceSession session, Terminal terminal, int channelNumber, ISmartcardServiceCallback callback) {
        ...
        try {
			...
			/* - snb */
			/* + snb log */
        	SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "Failed to register client callback");
			/* - snb log */
        }
    }
    ...
    public void binderDied() {
        ...
        try {
			/* + snb log */
            SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, Thread.currentThread().getName()
    ...
    public synchronized void close() throws CardException {
        ...
        if (isBasicChannel() && hasSelectedAid()) {
            try {
				/* + snb log */
            	SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_MIDDLE, "Close basic channel - Select with out AID ..."); 
				/* - snb log */
                terminal.select();
...
//#if defined(sec)
                ...
                try {
					/* + snb log */
                	SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, "Close basic channel - Exception : " + exp.getLocalizedMessage()); 
					/* - snb log */
...
//#endif
	...
	public byte[] transmit(byte[] command) throws CardException {
		...
		if (((command[0] & (byte) 0x80) == 0)
			...
			/* + snb GetUid */
        	if(command.length == 5 && command[0]==(byte)0xFF && command[1]==(byte)0xFF && command[2]==(byte)0xFF && command[3]==(byte)0xFF && command[4]==(byte)0xFF){
        		SmartcardService.log(CHANNEL_TAG + SmartcardService.LOG_LEVEL_MIDDLE,"yong command is getUid");
        		return getTerminal().GetUidEncapsulation();
        	}
			/* + snb GetUid */
		}
	...
	private byte setChannelToClassByte(byte cla, int channelNumber) {
		/* + snb for lingnan */
		if(4 == mMaxChannelType){
		/* - snb for lingnan */
		if (channelNumber < 4) {
			// b7 = 0 indicates the first interindustry class byte coding
			cla = (byte) ((cla & 0xFC) | channelNumber);
		} else {
			throw new IllegalArgumentException(
					"Channel number must be within [0..19]");
		}			
		}else if(20==mMaxChannelType){
		/* - snb for lingnan */
		if (channelNumber < 4) {
			// b7 = 0 indicates the first interindustry class byte coding
		...
		/* + snb for lingnan */
			...
			throw new IllegalArgumentException(
					"Max Channel number Type is neither 4 nor 20");
		}
		/* - snb for lingnan */
		return cla;
	}
    ...
    final class SmartcardServiceChannel extends ISmartcardServiceChannel.Stub {
		...
		public byte[] transmit(byte[] command, SmartcardError error)
            ...
            try {
				...
				/* + snb log */
            	SmartcardService.log(SmartcardService._TAG + SmartcardService.LOG_LEVEL_BOTTOM, "transmit Exception: " + e.getMessage() + " (Command: " + Util.bytesToString(command) + ")");
				/* - snb log */
                SmartcardService.setError(error, e);
                return null;
            }
